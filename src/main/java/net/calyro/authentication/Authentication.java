...
public class Authentication {
    ...
    private static ServerInfo limbo = ProxyServer.getInstance().getServerInfo("limbo-1");
    ...
    public static boolean shouldAutoLogin(ProxiedPlayer player) {
        ...
        if (AuthDB.getPremiumUUID(user) != null) {
            return true;
        } else return sessionTime != null && AuthDB.getLastSessionValidation(player.getUniqueId()) != -1 && LocalDateTime.ofInstant(Instant.ofEpochMilli(AuthDB.getLastSessionValidation(player.getUniqueId())), ZoneId.systemDefault()).plus(sessionTime).isAfter(LocalDateTime.now());
    }
