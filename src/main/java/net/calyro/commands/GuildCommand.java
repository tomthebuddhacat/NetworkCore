...
import net.calyro.utility.*;
import net.calyro.guilds.database.GuildDatabase;
import net.calyro.guilds.other.Color;
		...
		case 1:
			...
			case "online":
				if (Guild.inGuild(player)) {
					...
					if (ProxyServer.getInstance().getPlayer(UUID.fromString(guild.getLeader())) != null) {
						...
						player.sendMessage((ProxyServer.getInstance().getPlayer(UUID.fromString(guild.getLeader())) == null ? "§c●" : "§a●") + " " + CoreAPI.getPlayerRank(UUID.fromString(guild.getLeader())).getPrefix() + User.getUser(UUID.fromString(guild.getLeader())).retrieveLastKnownName());
					}
					...
					if (guildOfficers.size() > 0) {
						...
						for (String str : guildOfficers) {
							toSend += (ProxyServer.getInstance().getPlayer(UUID.fromString(str)) == null ? "§c●" : "§a●") + " " + CoreAPI.getPlayerRank(UUID.fromString(str)).getPrefix() + User.getUser(UUID.fromString(str)).retrieveLastKnownName();
						}
					...
					if (guildMembers.size() > 0) {
						...
						for (String str : guildMembers) {
							toSend += (ProxyServer.getInstance().getPlayer(UUID.fromString(str)) == null ? "§c●" : "§a●") + " " + CoreAPI.getPlayerRank(UUID.fromString(str)).getPrefix() + User.retrieveLastKnownName(UUID.fromString(str));
						}
						player.sendMessage(toSend);
					}
			...
			case "list":
				if (Guild.inGuild(player)) {
					...
					player.sendMessage("§1 ");
					player.sendMessage("§a                  -- Guild Master --");
					player.sendMessage((ProxyServer.getInstance().getPlayer(UUID.fromString(guild.getLeader())) == null ? "§c●" : "§a●") + " " + CoreAPI.getPlayerRank(UUID.fromString(guild.getLeader())).getPrefix() + User.retrieveLastKnownName(UUID.fromString(guild.getLeader())));
			...
			case "info":
				if (Guild.inGuild(player)) {
					...
					int progress = Math.round((guild.getExperience() - Level.getLevelFromLongXP(guild.getExperience()).experienceNeeded) * 100 / (Level.getNextLevel(
												  guild.getExperience()).experienceNeeded - Level.getLevelFromLongXP(guild.getExperience()).experienceNeeded));
			...
			case "create":
				if (!CoreAPI.getPlayerRank(player.getUniqueId()).isAboveOrEqual(Rank.VIP_PLUS)) {
					player.sendMessage(getDividerAqua());
					player.sendMessage("§cYou need atleast §aVIP§6+ §cto create a guild!");
					player.sendMessage(getDividerAqua());
					return;
				}
				if (Guild.inGuild(player)) {
		...
		case 2:
					...
					if (Guild.getGuildFromPlayer(player).getLeader().equals(player.getUniqueId().toString())) {
						if (ProxyServer.getInstance().getPlayer(args[1]) != null) {
							if (Guild.getGuildFromPlayer(player).getAllPlayers().contains(ProxyServer.getInstance().getPlayer(args[1]).getUniqueId().toString())) {
								...
								if (guild.getOfficer().contains(ProxyServer.getInstance().getPlayer(args[1]).getUniqueId().toString())
										|| guild.getLeader().equals(ProxyServer.getInstance().getPlayer(args[1]).getUniqueId().toString())) {
									player.sendMessage(getDividerAqua());
					...
					if (Guild.getGuildFromPlayer(player).getLeader().equals(player.getUniqueId().toString())) {
						if (ProxyServer.getInstance().getPlayer(args[1]) != null) {
							if (Guild.getGuildFromPlayer(player).getAllPlayers().contains(ProxyServer.getInstance().getPlayer(args[1]).getUniqueId().toString())) {
								...
								if (guild.getMembers().contains(ProxyServer.getInstance().getPlayer(args[1]).getUniqueId().toString())
										|| guild.getLeader().equals(ProxyServer.getInstance().getPlayer(args[1]).getUniqueId().toString())) {
									player.sendMessage(getDividerAqua());
			...
			case "chat":
				if (Guild.inGuild(player)) {
					...
					final String roleeTagggv = roleTag;
					...
					guild.getOnlinePlayers().forEach(onlinePlayer -> {
						onlinePlayer.sendMessage("§2Guild > " + player.getName() + " " + roleeTagggv + "§f: " + message);
					});
				} else {
					player.sendMessage(getDividerAqua());
					player.sendMessage("§cYou must be in a guild to use this command!");
			...
			case "accept":
				if (Guild.inGuild(player)) {
					if (ProxyServer.getInstance().getPlayer(args[1]) != null && PENDING_REQUESTS.containsKey(ProxyServer.getInstance().getPlayer(args[1]))
							&& PENDING_REQUESTS.get(ProxyServer.getInstance().getPlayer(args[1])).equals(Guild.getGuildFromPlayer(player))) {
			...
			case "invite":
			    if (Ignores.getIgnoredUsers(player.getUniqueId()).contains(CoreAPI.getProxyPlayer(args[0]).getUniqueId())) {
			    	player.sendMessage("§cYou've been ignored by this user.");
			    	return;
			    }
				if (Guild.inGuild(player)) {
					if (ProxyServer.getInstance().getPlayer(args[1]) != null) {
							...
							if (Guild.getGuildFromPlayer(player).getLeader().equals(player.getUniqueId().toString()) || Guild.getGuildFromPlayer(player).getOfficer().contains(player.getUniqueId().toString())) {
								if (Guild.getGuildFromPlayer(player).getAllPlayers().contains(ProxyServer.getInstance().getPlayer(args[1]).getUniqueId().toString())) {
									...
									if (PENDING_INVITES.containsKey(ProxyServer.getInstance().getPlayer(args[1])) && PENDING_INVITES.get(ProxyServer.getInstance().getPlayer(args[1])).equals(player)) {
										...
										ProxyServer.getInstance().getPlayer(args[1]).sendMessage(getDividerAqua());
										ProxyServer.getInstance().getPlayer(args[1]).sendMessage(Utilities.getRankFromPlayer(player) + player.getName() + " §ahas invited you to join their guild, §6" + Guild.getGuildFromPlayer(
													player).getName() + "§a!");
										ProxyServer.getInstance().getPlayer(args[1]).sendMessage(message);
										ProxyServer.getInstance().getPlayer(args[1]).sendMessage(getDividerAqua());

										player.sendMessage(getDividerAqua());
										player.sendMessage("§eYou invited " + Utilities.getRankFromPlayer(ProxyServer.getInstance().getPlayer(args[1])) + ProxyServer.getInstance().getPlayer(
